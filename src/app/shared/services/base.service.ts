...
export class BaseService<T> {
   ...
   */
  protected httpOptions = { headers: new HttpHeaders({'Content-Type':
        'application/json'}) };
  /**
   * @property http
   * @description
  ...
  * The endpoint for the resource. This is the URL path for the resource.
   ...
   * @method handleError
   * @description
   * Handles the error response from the server. It logs the error to the
   console and returns an observable with the error.
   * @param {HttpErrorResponse} error - The error response from the
   server.
   */
...
${error.error}`);
   ...
   * @returns {Observable<any>} - An observable with the response from the
   server.
   */
   ...
   * @returns {Observable<Array<T>>} - An observable with the array of
   resources.
   */
